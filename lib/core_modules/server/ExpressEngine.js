...
ExpressEngine.prototype.beginBootstrap = function(meanioinstance, database) {
  ...
  function themeHandler(req, res) {
    ...
    gfs.files.findOne({
      ...
      if (!file) {
        fs.createReadStream(config.root +
          '/bower_components/bootstrap/dist/css/bootstrap.css').pipe(
          res);
      } else {
        // streaming to gridfs
  ...
  var port = config.http ? config.http.port : config.port;
  console.error('Starting http listener for ' + host + ':' + port);
  ...
  if (config.https && config.https.port) {
    ...
    console.error('Starting https listener for ' + host + ':' + config.https.port);
    httpsServer.listen(config.https.port, host);
  }
